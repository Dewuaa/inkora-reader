  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        console.log('[Hianime Route] Standard fetch success:', JSON.stringify(res));
        if (!res || !res.sources || res.sources.length === 0) {
            throw new Error('No sources found (empty response)');
        }
        reply.status(200).send(res);
      } catch (err) {
        console.log('[Hianime Route] Standard fetch failed, trying Puppeteer fallback...');
        try {
            const fs = require('fs');
            const path = require('path');
            fs.writeFileSync(path.resolve(process.cwd(), 'route_debug.txt'), `Fallback triggered at ${new Date().toISOString()}\nCWD: ${process.cwd()}\n`);
            
            const HianimeScraper = require('../../services/HianimeScraper').default;
            console.log('[Hianime Route] HianimeScraper loaded');
            const res = await HianimeScraper.getEpisodeSources(episodeId);
            console.log('[Hianime Route] Fallback result:', JSON.stringify(res));
            reply.status(200).send(res);
        } catch (fallbackErr: any) {
            console.error('Fallback failed:', fallbackErr);
            reply
            .status(500)
            .send({ message: 'Something went wrong. Contact developer for help.', error: fallbackErr.message });
        }
      }
    },
  );
